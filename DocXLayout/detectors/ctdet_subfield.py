...
class CtdetDetector_Subfield(BaseDetector):
    ...
    def process(self, images, return_time=False):
        ...
        with torch.no_grad():
            ...
            #add sub
            dets_sub, inds_sub = ctdet_4ps_decode(hm_sub, wh_sub, reg=reg_sub, K=self.opt.K)
        ...
        for i in range(1):
            ...
            for k in range(len(dets[i])):
                if detection[i, k, 8] > self.opt.center_thresh:
                    debugger.add_4ps_coco_bbox(detection[i, k, :8], detection[i, k, -1], detection[i, k, 8],
        ...
        for j in range(1, self.num_classes + 1):
            for bbox in results[j]:
                if bbox[8] > self.opt.scores_thresh:
                    ...
                    # print(j-1)
                    debugger.add_4ps_coco_bbox(bbox, j - 1, bbox[8], show_txt=True, img_id='ctdet')
        debugger.save_all_imgs(image_name, './demo/')
